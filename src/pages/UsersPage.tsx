...
const UsersPage = () => {
  ...
  const Userlist = async () => {
    try {
      const res = await fetch("/data/alluserlist.json"); // or your API URL
  ...
  const filteredUsers = users.filter((user) => {
    ...
    switch (selectedFilter) {
      ...
      case "patient":
        return matchesSearch && user.type === "patient";
      case "students":
        return matchesSearch && user.type === "students";
      case "doctor":
        return matchesSearch && user.type === "doctor";
      default:
        return false;
    }
  });
                ...
                {paginatedUsers.map((user) => (
                  <tr key={user.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-4 whitespace-nowrap">
                      <Badge
                        variant={
                          user.type === "patient" ? "primary" : "secondary"
                        }
                      >
                        {user.type === "patient"
                          ? "Patient"
